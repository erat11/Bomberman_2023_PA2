 ...
 *
 * Copies of CStrAuto are fast (the cost is one increment of an integer counter). Thus move
 * is implemented as copying to save code length. Note the implementation of copy operator=.
 */
...
{
  // the parameter is not a reference. Instead, the right-hand side operand is copied
  // by means of the copy cons into x. The copying is cheap, the cost is one integer increment.
  // x is an independent object that is freed when op= completes.
  // If the contents is swapped, the destruction of x releases the data formely stored in
  // the left-hand side operand.
  //
  // Note the move op= is missing. Thus the compiler splits moves into copy/delete. The cost
  // comparison is:
  // copy op= + delete:
  //  copy parameter (copy cons):   1 x inc
  //  copy op=:                     swap
  //  destruction of parameter x:   1 x dec (+optionally free the contents if not shared anymore)
  //  destruction of the temporary: 1 x dec
  // move op=:
  //  move op=:                     swap
  //  destruction of the temporary: 1 x dec (+optionally free the contents if not shared anymore)
  // the difference is 1 x inc + 1 x dec
...
{
  // This operator no longer returns char &, i.e. this operator result cannot be the left-hand side of

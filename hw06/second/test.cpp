...
class CPanel : public CParent 
{
  public: 
    ...
    CPanel & add ( const CParent & x )
    {
      if ( x.getType() == 5 )
      {
        CPanel a ( x.getId(), pos.first, x.getPos().first );
      ...
      {
        CComboBox a ( x.getId(), this->getPos().first, x.getPos().second );
    ...
    vector<CParent> get() { return storage; }
  private:
...
class CWindow : public CParent
{
  public:
    ...
    CWindow & add ( const CParent & x )
    {
        if ( x.getType() == 5 )
        {
          CPanel * a = new CPanel ( x.getId(), pos.first, x.getPos().first );
          auto b = dynamic_cast<CPanel *> ( &x );
          for ( const auto & c : b->get() ) a->add( c );
          storage.push_back( a );
        }
        else if ( x.getType() == 4 ) 
